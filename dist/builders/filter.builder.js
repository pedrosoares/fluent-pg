      ...
      var parseFunction = function parseFunction(filter) {
        if (filter instanceof Object && !!filter.filter) {
          ...
          if (!!filter.args && filter.args.length > 0) {
            filter.args.forEach(function (argument, index) {
              values.push(argument);
              raw = raw.split("$".concat(index + 1)).join("$".concat(++i));
            });
          }

          return "".concat(_type, " ").concat(raw);
          ...
          var _type3 = _this2.typerize(filter.type);

          var _column = _this2.columnrize(filter.column);

          var _compare = _this2.comparize(filter.compare);

          values.push(filter.value);
          return "".concat(_type3, " ").concat(_column, " ").concat(_compare, " $").concat(++i);
        } else {
